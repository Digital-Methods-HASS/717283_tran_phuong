...
author: "Phuong Tran"
date: " created 4 October 2021, updated 5 October 2022"
...
ggplot (data = Western_Europe, aes (x = Year, y = homicides_per_100k, colour = Entity)) + 
  geom_smooth() +
  labs(title = "Homicide rate in Europe", x = "Year", y = "Homicide rate per 100,000") + 
  theme_minimal()

# I don't know why but my plot is not displayed on Rmarkdown presentation. I have attached all the images of the plots in the folder of this 4th homework on Github. 
```
...
```{r eval = FALSE}
ggplot (data = Western_Europe, aes (x = Year, y = homicides_per_100k, colour = Entity)) + geom_smooth() + 
...
```{r eval = FALSE}
library(tidyverse)
danish_monarch <- read_csv2("kings_1.csv")
danish_monarch <- na.omit(danish_monarch)
danish_monarch$End_date <- as.numeric(as.character(danish_monarch$End_date))
danish_monarch$Start_date <- as.numeric(as.character(danish_monarch$Start_date))
danish_monarch$Yearasruler <- as.numeric(as.character(danish_monarch$Yearasruler))
midyear = danish_monarch$End_date - (danish_monarch$End_date - danish_monarch$Start_date)/2
danish_monarch$midyear <- midyear
ggplot(data = danish_monarch, aes(x=midyear, y=Yearasruler)) + geom_smooth() +
  labs(title = "Reign Duration", x = "Year of reign", y = "Reign duration") +
  theme_minimal()
```
...
According to the data showing that the number of homicides rapidly decrease throughout the years, yes, I think we are more civilized today. 
